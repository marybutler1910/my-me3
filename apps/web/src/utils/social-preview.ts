...
function siteUrl(siteHandle: string, path = ""): string {
  const cleanHandle = siteHandle.trim().replace(/^@/, "") || "your-site";
  const cleanPath = path.replace(/^\/+/, "");
  return cleanPath ? `https://${cleanHandle}/${cleanPath}` : `https://${cleanHandle}`;
}
...
export function buildSocialPreviewContent(
  ...
  const url =
    postType === "social"
      ? siteUrl(siteHandle)
      : siteUrl(siteHandle, `${blogPath}/${post.slug}`);
  ...
  if (platform === "instagram" || platform === "instagram_business") {
    const igBody = truncate(
      [title, summary, `Link in bio: ${siteUrl(siteHandle).replace("https://", "")}`]
        .filter(Boolean)
        .join("\n\n"),
      360,
    );
  ...
  if (platform === "youtube") {
    const youtubeBody = truncate(
      [
        title,
        summary,
        `Catch the full post on ${siteUrl(siteHandle).replace("https://", "")}`,
      ]
        .filter(Boolean)
        .join("\n\n"),
      700,
    ...
    return {
      ...
      url: siteUrl(siteHandle, `${blogPath}/${post.slug}`),
  ...
  const linkedInBody = truncate(
    [
      title,
      summary,
      `Read the full post on ${siteUrl(siteHandle).replace("https://", "")}`,
    ]
      .filter(Boolean)
      .join("\n\n"),
    600,
